...
Proof.
  intros. destruct (id_eq_dec x x) as [_ | NE]; [reflexivity | now exfalso].
Qed.

Lemma neq_id : forall (T:Type) x y (p q:T), x <> y -> (if id_eq_dec x y then p else q) = q.
Proof.
  intros T x y p q NE. destruct (id_eq_dec x y); [contradiction | reflexivity].
Qed.

Lemma lt_gt_id_false : forall id1 id2 : id,
    id1 i> id2 -> id2 i> id1 -> False.
Proof.
  intros [n] [m] H1 H2. inversion H1; inversion H2; lia.
Qed.
...
Proof.
  intros [n] [m] H.
  assert (Hle : n <= m) by (inversion H; assumption).
...
Proof.
  intros [n] [m] NE. destruct (lt_eq_lt_dec n m) as [[H | H] | H].
  - right; constructor; lia.
  - exfalso; apply NE; subst; reflexivity.
...
Proof.
  intros [n] [m] Heq H. inversion Heq; subst. inversion H; lia.
Qed.
